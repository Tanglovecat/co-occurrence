...
for a_ls in df['a_ls'].values:
    for a in a_ls:
        val_ls.append(a)
adf = pd.DataFrame(
    pd.Series(val_ls).value_counts()).reset_index().rename(
        columns={
            'index': 'author',
            0: 'paper_num'})

# 所有作者列表
all_author_ls = adf['author'].values

# 如下是计算合作次数
author_pair_ls = []
for i in range(len(all_author_ls)):
    for j in range(i + 1, len(all_author_ls)):
        # 记录作者对，合作次数， 合作论文id 列表
        author_pair_ls.append({'author_pair': (
            all_author_ls[i], all_author_ls[j]), 'co-times': 0, 'co-paper_id': []})
...
for i, author_pair in enumerate(author_pair_ls):
    for j, a_ls in enumerate(paper_authors_ls):
        ...
        ## 合作次数 +1
        # 合作论文id列表记录该论文id
        if (author_pair['author_pair'][0] in a_ls) & (
                author_pair['author_pair'][1] in a_ls):
...
apdf['co-occurrence_val'] = apdf['co-times']**2 / \
    apdf['authors_paper_num'].apply(lambda x: x[0] * x[1])

# 存储所有作者之间的合作次数与共现系数
apdf.to_csv('all_co-occurrence.csv')
